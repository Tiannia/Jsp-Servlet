...
public class LMONKEY_USERDao {
    public static int insert(LMONKEY_USER u) {
        String sql = "insert into shopuser values(?, ?, ?, ?, DATE_FORMAT(?,'%y-%m-%d'), ?, ?, ?, ?, ?)";
        Object[] params = {
                u.getUSER_ID(),
                u.getUSER_NAME(),
                u.getUSER_PASSWORD(),
    ...
    public static int update(LMONKEY_USER u) {
        String sql = "update shopuser set USER_NAME=?, USER_PASSWORD=?, USER_SEX=?, USER_BIRTHDAY=DATE_FORMAT(?,'%Y-%m-%d'), USER_IDENITY_CODE=?, USER_EMAIL=?, USER_MOBILE=?, USER_ADDRESS=?,USER_STATUS=? where USER_ID = ?";
        Object[] params = {
                u.getUSER_NAME(),
                u.getUSER_PASSWORD(),
                u.getUSER_SEX(),
    ...
    public static int[] totalPage(int count, String keyword) {
        ...
        try {
            if (keyword != null) {
                String sql = "select count(*) from shopuser where USER_ID like ?";
                ps = conn.prepareStatement(sql);
    ...
    public static ArrayList<LMONKEY_USER> selectAll(int cpage, int count, String keyword) {//int cpage, int count
        ...
        try {
            String sql = "";
            if (keyword != null) {
                sql = "SELECT * from shopuser where USER_ID like ? order by USER_BIRTHDAY desc limit ? , ?";
                //desc 是降序排序
                ps = conn.prepareStatement(sql);
                ps.setString(1, "%" + keyword + "%"); //"%" 可用于定义通配符（模式中缺少的字母）
                ps.setInt(2, (cpage - 1) * count);
                ps.setInt(3, count);
                //sql = "select * from shopuser where USER_ID like '%" + keyword + "%'  order by USER_BIRTHDAY";
            } else {

                sql = "select * from shopuser order by USER_BIRTHDAY desc limit ?, ?";
                ps = conn.prepareStatement(sql);
    ...
    public static LMONKEY_USER selectByID(String id) {
        ...
        try {
            String sql = "select m.*, DATE_FORMAT(m.USER_BIRTHDAY,'%Y-%m-%d') birt from shopuser m where USER_ID = ?";
            ps = conn.prepareStatement(sql);
            ps.setString(1, id);
